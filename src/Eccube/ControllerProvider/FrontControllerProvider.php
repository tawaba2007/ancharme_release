...
class FrontControllerProvider implements ControllerProviderInterface
{
    public function connect(Application $app)
    {
        ...
        // 商品一覧ページ
        $c->match('/block/categoryTops', '\Eccube\Controller\Block\categoryTops::index')->bind('block_categoryTops');
        $c->match('/block/categoryOuter', '\Eccube\Controller\Block\categoryOuter::index')->bind('block_categoryOuter');
        $c->match('/block/categoryBottoms', '\Eccube\Controller\Block\categoryBottoms::index')->bind('block_categoryBottoms');
        $c->match('/block/categoryDress', '\Eccube\Controller\Block\categoryDress::index')->bind('block_categoryDress');
        $c->match('/block/categorySale', '\Eccube\Controller\Block\categorySale::index')->bind('block_categorySale');
        $c->match('/block/pick_up', '\Eccube\Controller\Block\pick_up::index')->bind('block_pick_up');
        $c->match('/block/tmp_itemList_all_category16', '\Eccube\Controller\Block\tmp_itemList_all_category16::index')->bind('block_tmp_itemList_all_category16');

        //追加 10/23
        $c->match('/'.'{route}', '\Eccube\Controller\UserDataController::index')->assert('route', '[0-9a-zA-Z_]+')->bind('user_data');

        // lpitem
        $c->match('/block/tmp_lpitem01_a', '\Eccube\Controller\Block\tmp_lpitem01_a::index')->bind('block_tmp_lpitem01_a');
        $c->match('/block/tmp_lpitem01_b', '\Eccube\Controller\Block\tmp_lpitem01_b::index')->bind('block_tmp_lpitem01_b');
